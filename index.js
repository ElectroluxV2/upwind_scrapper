...
const pending_open_skiff_calendars = Array
    .from({ length: 3 }, (v, i) => `https://openskiff.pl/php/getCalendar.php?year=${i + 2020}`)
    .map(get);

const open_skiff_calendars = await Promise.all(pending_open_skiff_calendars);

console.log(open_skiff_calendars
    .map(({regattas}) => regattas)
    .flat()
    .map(({name, place, dateBegin, dateEnd, multiplier}) => ({
        name,
        begin_date: to_pg_date(polish_date_to_date(dateBegin)),
        end_date: to_pg_date(polish_date_to_date(dateEnd)),
        cup_multiplier: multiplier === '0.0' ? null : multiplier,
        place_id: filtered_places
            .map((v, index) => ({
                name: v,
                similarity: string_similarity(place, v),
                id: index + 1
            }))
            .sort((a, b) => a.similarity > b.similarity ? -1 : 1)
            .find((v, index) => index === 0)
    ...
    .map(({name, place_id, begin_date, end_date, cup_multiplier}) => `(0, ${place_id}, 0, ${cup_multiplier}, '${begin_date}', '${end_date}', '${name}')`)
    .join(',\n')
)
